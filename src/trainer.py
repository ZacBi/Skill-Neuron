...
class trainer:
    def __init__(self, args):
        ...
        self.optimizer = Adam(optimizer_parameters,
                              lr=self.args.lr, eps=1e-6, betas=(0.9, 0.98))
    ...
    def mlm_train_step(self, labeled_batch, return_length=False) -> torch.Tensor:
        ...
        if (len(datasetRow[self.args.task_type]) > 2):
            sentences = [sentences[_] + labeled_batch[datasetRow[self.args.task_type][1]][_]
                         for _ in range(len(sentences))]
        sentences = self.model.tokenizer(
            sentences, padding=True, truncation=True,  return_tensors="pt", max_length=512 - self.args.prompt_size)
    ...
    def train(self,
        ...
        train_dataloader = DataLoader(
            train, sampler=train_sampler, batch_size=self.args.bz)
        ...
        self.scheduler = get_linear_schedule_with_warmup(
            self.optimizer, num_warmup_steps=t_total*0.006, num_training_steps=t_total)
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                self.model.train()
                labels = batch[datasetRow[self.args.task_type]
                               [-1]].to(self.device)
                loss = self.criterion(
                    self.mlm_train_step(batch), labels.long())
                del labels
                if n_gpu > 1:
                    loss = loss.mean()
                loss.backward()
                ...
                if global_step % eval_every_step == 0:
                    ...
                    # Shortcut
                    dev_scores = origin_dev_scores[list(
                        origin_dev_scores.keys())[-1]]
                    if dev_scores >= best_dev_scores:
                        ...
                        self.logger.info(
                            "Saving trained model at {}...".format(self.args.save_to))
                        self.logger.info("best_dev_perf: %.4f | best_global_step: %d" % (
                            best_dev_scores, best_global_step))
    ...
    def eval_dev(self, dev_data):
        ...
        try:
            scores = glue_compute_metrics(
                self.args.task_type, predictions, results['labels'])
        except:
            scores = {"acc": np.sum(
                predictions == results['labels'])/predictions.shape[0]}
        return scores
    ...
    def eval(self,
        ...
        eval_dataloader = DataLoader(
            valid, sampler=eval_sampler, batch_size=self.args.bz)
    ...
    def probe(self,
        ...
        eval_dataloader = DataLoader(
            eval_data, sampler=eval_sampler, batch_size=self.args.bz)
    ...
    def probe_pos(self,
        ...
        def process(hidden, label, length):
            activation.append(hidden[pos[0]][0][:, :, pos[1]]
                              [:, : self.args.prompt_size].detach().cpu().numpy())
    ...
    def probe_avg(self,
                  eval_data) -> Dict: 
        ths = np.zeros(
            (self.model.layer_num, self.model.prompt_size, self.model.layer_width))
        if (self.model.prompt_type == "MLM"):
            # Bert, Roberta
            def process(hidden, label, length):
                """
                Args:
                    hidden (List[List[torch.Tensor]]): 隐层, 每一层一个hidden_states, 即hidden[k][0] 为
                """
        ...
        self.probe(eval_data, process)
        # NOTE: activation baseline for promt on each layer (take average)
        ...
        def process(hidden, label, length):
            for k in range(self.model.layer_num):
                ...
                for snt in range(len(length)):
                    ths[k] += hidden[k][0][snt, offset:offset +
                                           length[snt], :].mean(axis=0)
        self.probe(eval_data, process)
        ...
        def process(hidden, label, length):
            for k in range(self.model.layer_num):
                ...
                for snt in range(length.shape[0]):
                    pred = hidden[k][0][snt, offset:offset +
                                        length[snt], :].mean(axis=0)
                    pred = (pred > ths[k])
